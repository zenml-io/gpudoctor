"""Amazon ECR Public Gallery API client for fetching AWS DLC image tag metadata.

AWS Deep Learning Containers are hosted in ECR Public at:
  public.ecr.aws/deep-learning-containers/{repo}:{tag}

This client talks to the OCI registry API exposed at public.ecr.aws.
...
ECR Public uses anonymous access for public images but requires token-based auth
...
ECR_NAMESPACE = "deep-learning-containers"
TIMEOUT = 30.0
...
def _get_manifest(
    ...
    url = f"{ECR_REGISTRY}/v2/{image_name}/manifests/{reference}"
...
def _parse_single_manifest(
    ...
    if isinstance(config_digest, str) and config_digest:
        url = f"{ECR_REGISTRY}/v2/{image_name}/blobs/{config_digest}"
        try:
...
def _parse_index_manifest(
    ...
    child_url = f"{ECR_REGISTRY}/v2/{image_name}/manifests/{digest}"
...
def get_tag(repo: str, tag: str) -> TagInfo | None:
    """Fetch metadata for a specific AWS DLC image tag.

    Args:
        repo: Repository name (e.g., "pytorch-training", "tensorflow-inference")
        tag: Tag name (e.g., "2.7.1-gpu-py312-cu128-ubuntu22.04-ec2")

    Returns:
        TagInfo if found and parsed successfully, otherwise None.
    """
    image_name = f"{ECR_NAMESPACE}/{repo}"
    ...
    with httpx.Client(timeout=TIMEOUT) as client:
        try:
            ...
            logger.warning(
                "Failed to fetch manifest for public.ecr.aws/%s:%s: %s",
                image_name,
                tag,
                e,
            )
        ...
        try:
            ...
            logger.warning(
                "Failed to parse manifest data for public.ecr.aws/%s:%s: %s",
                image_name,
                tag,
                e,
            )
