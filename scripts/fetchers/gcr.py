"""Google Container Registry (GCR) API client for fetching GCP DLC image tag metadata.

GCP Deep Learning Containers are hosted at:
  gcr.io/deeplearning-platform-release/{repo}:{tag}
  us-docker.pkg.dev/deeplearning-platform-release/gcr.io/{repo}:{tag}

This client talks to the OCI registry API exposed at gcr.io.
...
GCR allows anonymous access for public images but may require token-based auth.

Repo name format examples:
  - pytorch-gpu.2-2  (PyTorch 2.2 GPU)
  - tf-gpu.2-15      (TensorFlow 2.15 GPU)
  - base-cu121       (Base CUDA 12.1)
...
GCR_PROJECT = "deeplearning-platform-release"
TIMEOUT = 30.0
...
def _get_bearer_token(client: httpx.Client, auth_header: str, image_name: str) -> str | None:
    ...
    try:
        ...
        logger.warning("Failed to obtain bearer token for %s: %s", image_name, e)
        return None
    except ValueError as e:
        logger.debug("Failed to decode bearer token response for %s: %s", image_name, e)
        return None

    token = data.get("token") or data.get("access_token")
    if not isinstance(token, str) or not token:
        logger.warning("Bearer token response for %s did not contain a token", image_name)
        return None
...
def _get_manifest(
    ...
    url = f"{GCR_REGISTRY}/v2/{image_name}/manifests/{reference}"
    ...
    try:
        ...
        logger.debug("Failed to decode manifest response for %s:%s: %s", image_name, reference, e)
        raise
...
def _parse_single_manifest(
    ...
    if isinstance(config_digest, str) and config_digest:
        url = f"{GCR_REGISTRY}/v2/{image_name}/blobs/{config_digest}"
        try:
            ...
            logger.debug("Failed to fetch config blob for %s@%s: %s", image_name, config_digest, e)
...
def _parse_index_manifest(
    ...
    child_url = f"{GCR_REGISTRY}/v2/{image_name}/manifests/{digest}"
...
def get_tag(repo: str, tag: str) -> TagInfo | None:
    """Fetch metadata for a specific GCP DLC image tag.

    Args:
        repo: Repository name (e.g., "pytorch-gpu.2-2", "tf-gpu.2-15")
        tag: Tag name (e.g., "latest", "py310", "m96")

    Returns:
        TagInfo if found and parsed successfully, otherwise None.
    """
    image_name = f"{GCR_PROJECT}/{repo}"
    ...
    with httpx.Client(timeout=TIMEOUT) as client:
        try:
            ...
            logger.warning(
                "Failed to fetch manifest for gcr.io/%s:%s: %s",
                image_name,
                tag,
                e,
            )
        ...
        try:
            ...
            logger.warning(
                "Failed to parse manifest data for gcr.io/%s:%s: %s",
                image_name,
                tag,
                e,
            )
