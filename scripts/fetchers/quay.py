"""Quay.io registry API client for fetching image tag metadata.

This client talks directly to the OCI registry API exposed at quay.io.
...
Quay.io also provides a REST API at /api/v1 for tag listing, but we use
the standard OCI endpoints for consistency with other fetchers.
"""
...
def _get_bearer_token(client: httpx.Client, auth_header: str, image_name: str) -> str | None:
    ...
    try:
        ...
        logger.warning("Failed to obtain bearer token for quay.io/%s: %s", image_name, e)
        return None
    except ValueError as e:
        logger.debug("Failed to decode bearer token response for quay.io/%s: %s", image_name, e)
        return None

    token = data.get("token") or data.get("access_token")
    if not isinstance(token, str) or not token:
        logger.warning("Bearer token response for quay.io/%s did not contain a token", image_name)
        return None
...
def _get_manifest(
    ...
    """
    url = f"{QUAY_REGISTRY}/v2/{image_name}/manifests/{reference}"
    ...
    try:
        ...
        logger.debug("Failed to decode manifest response for quay.io/%s:%s: %s", image_name, reference, e)
        raise
...
def _parse_single_manifest(
    ...
    if isinstance(config_digest, str) and config_digest:
        url = f"{QUAY_REGISTRY}/v2/{image_name}/blobs/{config_digest}"
        try:
            ...
            logger.debug("Failed to fetch config blob for quay.io/%s@%s: %s", image_name, config_digest, e)
...
def _parse_index_manifest(
    ...
    child_url = f"{QUAY_REGISTRY}/v2/{image_name}/manifests/{digest}"
...
def get_tag(org: str, repo: str, tag: str) -> TagInfo | None:
    """Fetch metadata for a specific Quay.io image tag.

    Args:
        org: Quay.io organization or user (e.g., "jupyter")
        repo: Repository/package name (e.g., "datascience-notebook")
        tag: Tag name
    ...
    with httpx.Client(timeout=TIMEOUT) as client:
        try:
            ...
            logger.warning(
                "Failed to fetch manifest for quay.io/%s:%s: %s",
                image_name,
                tag,
                e,
            )
        ...
        try:
            ...
            logger.warning(
                "Failed to parse manifest data for quay.io/%s:%s: %s",
                image_name,
                tag,
                e,
            )
